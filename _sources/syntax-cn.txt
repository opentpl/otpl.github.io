...
这样模板引擎将会从上下文上去查找变量 ``foobar`` 然后打印出来。变量可以使用 ``.`` 来访问属性，和 `JavaScript`_ ([1]以下等同) 一样，也可以用 ``[]``::

{{ foo.bar }}
{{ foo['bar'] }}

这两种方式的结果都是一样的。
如果这个变量的值是 ``null``，什么也不会被输出。同样的，如果引用了 ``null`` 的属性，也不会输出任何值（也不会报错）。
下面的这些操作当 ``foo == null`` 的时候，都不会输出任何值::
...
- ``boolean`` 布尔类型。它由 ``true`` 和 ``false`` 两个固定值表示。

- ``number`` 数值类型。数值类型分为整数 ``integer`` 和浮点数 ``float`` 表示。 
...
``注：`` 数值类型目前精度未最终确认，如JavaScript的整数和JAVA的整数的差异，将由实现引擎自主决定。
OTPL致力于打造全平台通用所以在实现上须考虑这种差异性带来的问题。
...
``注：`` 在某些OTPL引擎实现中可能会对模板进行优化如：对空白行进行删除。
只有在如 ``{{'strings...'}}`` 中的字符串是严格按模板输出的。
...
OTPL采用的是单次渲染一个全局作用域，子模板与父模板使用同一个上下文，但有一个特殊的作用域那就是`块`调用（参见块）。
...
函数
=======
函数的使用跟JavaScript的方法是一的简单如：``{{now()}}`` ，你也可以使用变量上提供的成员方法::
...
过滤器
======
过滤器的使用方法是使用Unix 管道。语法： ``{{foobar | filter=args}}`` ,如::

    {{now() | time='yyyy-MM-dd'}}

输出将是一个类似 ``2016-07-30`` 的一个日期字符串(具体的值根据你的系统日期决定)。

``args`` 表示参数，参数个数为大于等于0个，只有一个参数时：``=arg`` ;大于1个参数需要使用括号如：``=(arg1,arg2,argN)`` 而没有参数时只需要过滤器名就行了：``{{now() | time}}`` 。

过滤器是支持多个同时调用，如：``{{foobar | filter1=args | filter2=args |filterN=args}}`` 。

过滤器是一种特殊的函数，它也可以像其它函数一样的调用，如：``{{time(now(),'yyyy-MM-dd')}}``。
...
if 检查一个条件，让你可以选择性的输出内容，它和大多编程语言中的 ``if`` 语句完全一致。
::
